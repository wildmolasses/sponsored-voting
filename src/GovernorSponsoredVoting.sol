...
abstract contract GovernorSponsoredVoting is Governor, BasePaymaster {
        ...
        returns (bytes memory context, uint256 validationData)
    {
        ...
        (
            address _governor,
            uint256 _value,
            bytes memory _voteOp
        ) = _decodeExecute(_userOp.callData);
    ...
    function _decodeExecute(
        bytes calldata _execOp
    )
        internal
        pure
        returns (address _governor, uint256 _value, bytes memory _op)
    {
        ...
        (_governor, _value, _op) = abi.decode(
            _execOp[4:],
            (address, uint256, bytes)
        );
    }

    function _decodeCastVote(
        bytes memory _voteOp
    ) internal pure returns (uint256 _proposalId, uint8 _support) {
        require(bytes4(_voteOp.slice(0, 4)) == Governor.castVote.selector);
        (_proposalId, _support) = abi.decode(
            _voteOp.slice(4, _voteOp.length - 4),
            (uint256, uint8)
        );
    }
}
